...
const registerClient = async () => {
  if (!clientForm.value.name.trim() || !clientForm.value.phone.trim()) {
    clientError.value = 'Por favor completa tu nombre y número.'
    return
  }
...
const copyPhone = async () => {
  await navigator.clipboard.writeText(provider.value?.phone || '')
      ...
      <div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
        ...
        <div class="px-6 py-5 space-y-3">
          ...
          <div>
            ...
            <input v-model="clientForm.phone" type="tel" inputmode="numeric" placeholder="Ej. 7711234567"
              class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-green" />
          </div>
      ...
      <div class="pt-12 px-6 pb-6">
        ...
        <div class="mt-5 flex flex-wrap gap-3 items-center">
          ...
          <div v-if="telLink" class="hidden md:flex items-center gap-2">
            ...
            <div v-else class="inline-flex items-center gap-2 border border-brand-green text-brand-green px-4 py-2.5 rounded-xl text-sm font-medium">
              <span>{{ provider.phone }}</span>
              <button @click="copyPhone" class="ml-1 text-xs px-2 py-0.5 rounded bg-brand-green text-white hover:bg-brand-lightGreen transition-colors">
